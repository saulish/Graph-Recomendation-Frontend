import {back_port, return_index, return_menu, logout, backendUrl, frontUrl, front_port} from "./redirects.js"
async function main(){
    await setDatos();
}
...
async function setDatos() {
    ...
    console.log("Nueva a consultar: "+url)
    ...
    if(!data.ok){
        ...
        alert("read the errors, menso")
        switch (data.error){
            case("invalid token"):
                window.location.href=frontUrl
                break
            default:
                console.error("Playlist not available!! "+data.error)
                break
        }
        return
    }
...
async function analizarPlaylist(id, total){
    const res= await fetch(backendUrl+"analizePlaylist?id="+encodeURIComponent(id),{
    credentials: "include",
    method: "GET"
    })
    if (!res.ok) {
    ...
    while (!readerDone) {
        ...
        // separar por líneas y procesar las completas
        const lines = buffer.split("\n");
        // dejamos la última como remanente — puede estar incompleta
        buffer = lines.pop();
        ...
        try {
            ...
            // manejar objeto parcial
            if (obj.done) {
            console.log("Stream finished");
            } else {
            console.log("Batch recibido:", obj);
            }
        } catch (err) {
    ...
    // procesar cualquier remanente (última línea si no terminó con \n)
    if (buffer) {
        try {
        const obj = JSON.parse(buffer);
        console.log("Último chunk:", obj);
        handleBatch(obj);
        } catch (err) {
        console.error("Error parseando remanente:", buffer, err);
        }
    }
